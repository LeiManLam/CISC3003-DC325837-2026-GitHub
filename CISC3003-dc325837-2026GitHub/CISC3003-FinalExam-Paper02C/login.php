...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $mysqli->prepare(
            'SELECT id, full_name, password_hash, email_verified, created_at FROM users WHERE email = ? LIMIT 1'
        );
        if ($stmt) {
            ...
            if ($row && password_verify($password, $row['password_hash'])) {
                if (!(int)$row['email_verified']) {
                    ...
                    $_SESSION['c_user_id'] = (int)$row['id'];
                    $_SESSION['c_name'] = $row['full_name'];
                    $_SESSION['c_since'] = $row['created_at'];
                    header('Location: dashboard.php');
                    exit;
                }
            } else {
...
<body>
    ...
    <main>
        <div class="card">
            ...
            <p class="hint"><a href="forgot_password.php">Forgot password?</a></p>
        </div>
    </main>
